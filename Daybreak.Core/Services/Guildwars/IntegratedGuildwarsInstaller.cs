...
) : IGuildWarsInstaller
{
    ...
    private static readonly string StagingFolder = PathUtils.GetAbsolutePathFromRoot(
        "GuildWarsCache"
    );

    private static readonly ProgressUpdate ProgressFailed = new(1, "Failed");
    private static readonly ProgressUpdate ProgressStartingExecutable = new(
        1,
        "Starting Executable"
    );
    private static readonly ProgressUpdate ProgressFinished = new(1, "Installation finished");

    private static ProgressUpdate ProgressUnpacking(double progress) =>
        new(progress, "Unpacking files");

    private static ProgressUpdate ProgressDownloading(double progress) =>
        new(progress, "Downloading files");
    ...
    public async IAsyncEnumerable<GuildWarsUpdateResponse> CheckAndUpdateGuildWarsExecutables(
        List<GuildWarsUpdateRequest> requests,
    ...
    )
    {
        ...
        foreach (var request in requests)
        {
            if (request.ExecutablePath!.IsNullOrWhiteSpace())
            {
                ...
                yield return new GuildWarsUpdateResponse
                {
            ...
            var scopedLogger = this.logger.CreateScopedLogger(
                flowIdentifier: request.ExecutablePath!
            );
            ...
            if (
                await this.GetVersionId(request.ExecutablePath, request.CancellationToken)
                    is int version
            ...
            try
            {
                success = await this.UpdateGuildwars(
    ...
    public async Task<bool> UpdateGuildwars(
        string exePath,
    ...
    public async Task<bool> InstallGuildwars(
    ...
    )
    {
        return await new TaskFactory()
            .StartNew(
                _ =>
                {
                    return this.InstallGuildwarsInternal(
                        destinationPath,
                        progress,
                        cancellationToken
                    );
    ...
    public async Task<int?> GetVersionId(string executablePath, CancellationToken cancellationToken)
    {
        ...
        catch (Exception e)
        {
            scopedLogger.LogError(
                e,
                "Failed to get version using new method, trying legacy method"
            );
            try
            {
    ...
    private async Task<bool> UpdateGuildwarsInternal(
        string exePath,
        IProgress<ProgressUpdate> progress,
        CancellationToken cancellationToken
    )
    ...
    private async Task<string?> FetchLatestGuildwarsInternal(
        IProgress<ProgressUpdate> progress,
        CancellationToken cancellationToken
    )
    {
        ...
        try
        {
            ...
            var tempName = Path.Combine(
                StagingFolder,
                CompressedTempExeName.Replace(VersionPlaceholder, manifest.LatestExe.ToString())
            );
            var cacheName = Path.Combine(
                StagingFolder,
                UncompressedTempExeName.Replace(VersionPlaceholder, manifest.LatestExe.ToString())
            );
            if (
                File.Exists(cacheName)
            ...
            (var downloadResult, var expectedFinalSize) = await this.DownloadCompressedExecutable(
                tempName,
                guildWarsClient,
                context,
                manifest,
    ...
    private async Task<bool> InstallGuildwarsInternal(
        string destinationPath,
        IProgress<ProgressUpdate> progress,
        CancellationToken cancellationToken
    )
    ...
    )
    {
        ...
        var maybeStream = await guildWarsClient.GetFileStream(
            context,
            manifest.LatestExe,
            0,
            cancellationToken
        ...
        do
        {
            ...
            progress.Report(
                ProgressDownloading((double)downloadStream.Position / downloadStream.Length)
            );
        } while (readBytes > 0);
    ...
    )
    {
        ...
        try
        {
            ...
            using var finalExeStream = new FileStream(
                exeName,
                FileMode.Create,
            ...
            {
                progress.Report(
                    ProgressUnpacking((double)finalExeStream.Length / expectedFinalSize)
                );
                    ...
                    else
                    {
                        ...
                        if (backtrack >= finalExeStream.Length)
                        {
                            throw new InvalidOperationException(
                                "Failed to decompress executable. backtrack >= finalExeStream.Length"
                            );
                        }
