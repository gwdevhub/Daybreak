...
public sealed class TradeMessageViewModel(
    INotificationService notificationService,
    IViewManager viewManager,
    ITradeAlertingService tradeAlertingService)
    : ViewModelBase<TradeMessageViewModel, TradeMessageView>
{
    ...
    public TradeAlert? Alert { get; private set; }

    public override ValueTask Initialize(CancellationToken cancellationToken)
    {
        if (this.tradeAlertingService.TradeAlerts.FirstOrDefault(t => t.Id == this.View?.AlertId) is not TradeAlert alert)
        {
